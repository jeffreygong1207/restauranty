...
function Diner() {
  const { Ic } = RFCore;
  const [selected, setSelected] = React.useState('refill');
  const [submitted, setSubmitted] = React.useState(false);
  const opts = [
    { id: 'confirm', label: 'Confirm I’m coming', desc: 'Locks the reservation. We’ll see you at 8:15 PM.' },
      ...
      <div className="page" style={{ background: 'var(--bg-sunken)', minHeight: 'calc(100vh - 56px)' }}>
        <div className="diner-shell">
          <div className="success-card" style={{ textAlign: 'center' }}>
            <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--success)', color: 'white', display: 'grid', placeItems: 'center', margin: '0 auto 14px' }}><Ic.check/></div>
            <h2 style={{ margin: '0 0 6px', fontSize: 22, letterSpacing: '-0.02em' }}>Waitlist refill requested</h2>
            <p style={{ margin: 0, color: 'var(--ink-2)' }}>We’ll text you within 15 minutes. If a verified diner takes your table, your $40 fee will be waived automatically.</p>
            ...
            <div style={{ textAlign: 'left' }}>
              ...
              <div className="kv"><span className="k">If not refilled by 7:30</span><span className="v">Standard policy applies</span></div>
            </div>
    ...
    <div className="page" style={{ background: 'var(--bg-sunken)', minHeight: 'calc(100vh - 56px)' }}>
      <div className="diner-shell">
        <div style={{ textAlign: 'center', marginBottom: 18 }}>
          <h1 style={{ fontSize: 24, letterSpacing: '-0.025em', margin: '0 0 6px', fontFamily: 'var(--font-display)', fontWeight: 500 }}>
        ...
        <div className="diner-card">
          ...
          <div style={{ marginTop: 14, padding: 12, background: 'var(--bg-sunken)', borderRadius: 8, fontSize: 12.5, color: 'var(--ink-3)', borderLeft: '3px solid var(--accent)' }}>
            <b style={{ color: 'var(--ink-2)' }}>Why we recommend this:</b> If the restaurant fills your table, your cancellation fee is waived per Katsuya’s policy. No resale, no markup — verified diners only.
          </div>
          ...
          <button className="btn ghost sm" style={{ width: '100%', marginTop: 6 }}>Not now — remind me at 6:00</button>
        </div>

        <div style={{ textAlign: 'center', marginTop: 16, fontSize: 11.5, color: 'var(--ink-4)' }}>
          ResoFlow follows Katsuya’s policies. We never resell or markup your reservation.
        </div>
      </div>
    </div>
  );
